...
with picamera.PiCamera() as camera:
    ...
    camera.start_recording('/dev/null', format='yuv', splitter_port=2)
    start = time.time()
    N_frames = 0
    while True:
        output = np.empty((winH, w), dtype=np.uint8)
        camera.capture(output, 'yuv', use_video_port=True, truncate_rear=False)
        ts = camera.frame.timestamp
        cv2.imwrite('/dev/shm/' + str(int(ts * 1e6)).zfill(10) + '.bmp', output)
        N_frames += 1
