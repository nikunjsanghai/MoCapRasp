...
with picamera.PiCamera(resolution=(w, h), framerate=fps) as camera:
    time.sleep(2)  # warm-up time for the camera
    start = time.time()
    stream = picamera.array.PiYUVArray(camera)
    for _ in camera.capture_continuous(stream, format='yuv', use_video_port=True):
        # convert YUV to RGB for OpenCV
        frame = cv2.cvtColor(stream.array, cv2.COLOR_YUV2BGR)
